...
import { audioPlayer, dBfsToLinear } from '@modules/audioPlayer/audioPlayer'
...
const DOOR_OPEN_DIST = 5      // player distance (m) at which doors fully open
const DOOR_AJAR_DIST = 6.9     // player distance (m) at which doors start cracking open
const DOOR_CLOSE_DIST = 7     // player distance (m) at which doors fully close
...
const DOOR_TWEEN_DURATION = 1500  // ms
...
export class Portal {
  ...
  private audioAmb: Entity | undefined
  private portalBody: Entity
  private enabled: boolean

  // Entities only created when enabled
  private ajarTrigger: Entity | undefined
  private openTrigger: Entity | undefined
  private closeTrigger: Entity | undefined
  private thresholdTrigger: Entity | undefined
  private portalLight: Entity | undefined
  private infoCard: Entity | undefined
  private infoBg: Entity | undefined
  private infoLabel: Entity | undefined
  private thumbPlane: Entity | undefined
  private usersIcon: Entity | undefined

  private frameArrows: Entity | undefined
  private disabledCollider: Entity | undefined
  ...
  constructor(options: PortalOptions) {
    ...
    // onLeaveScene — registered once for the lifetime of this portal, gated by this.enabled
    onLeaveScene((userId) => {
    ...
    if (this.enabled) {
      this.initInteractive(options)
    } else {
      this.addDisabledCollider()
    }

    this.update(options)
  }

  // Creates all interactive entities: triggers, light, layers, info card.
  // Idempotent — returns immediately if already initialized.
  private initInteractive(options: PortalOptions): void {
    if (this.ajarTrigger) return

    this.removeDisabledCollider()

    // Frame arrows — shown only when enabled
    ...
    GltfContainer.create(this.frameArrows, { src: 'assets/models/portals/portalFrameArrows.glb' })
    ...
    // Ajar trigger (6.9 m) — tracks who's inside for AJAR state
    ...
    // Open trigger (5 m) — tracks who's inside for OPEN state
    ...
    // Close trigger (7 m) — hysteresis gate: door only closes when this set empties
    ...
    this.layers = Array.from({ length: LAYER_COUNT }, (_, i) => {
      ...
      GltfContainer.create(entity, { src: 'assets/models/portals/portalLayer2.glb' })
    ...
    GltfContainer.createOrReplace(this.infoBg, { src: 'assets/models/portals/event_card_bottom_tall.glb' })
    ...
    MeshRenderer.setPlane(this.usersIcon)
    Material.setPbrMaterial(this.usersIcon, {
      texture: Material.Texture.Common({ src: 'images/portals/PlayersFilledBlack.png' }),
      transparencyMode: MaterialTransparencyMode.MTM_ALPHA_TEST,
      castShadows: false,
    })
  }

  // Destroys all interactive entities and resets portal to closed/static state.
  private cleanupInteractive(): void {
    if (this.ajarTrigger) {
      ...
      engine.removeEntity(this.ajarTrigger)
      this.ajarTrigger = undefined
    }
    if (this.openTrigger) {
      ...
      engine.removeEntity(this.openTrigger)
    ...
    if (this.closeTrigger) {
      ...
      engine.removeEntity(this.closeTrigger)
      this.closeTrigger = undefined
    }
    if (this.thresholdTrigger) {
      triggerAreaEventsSystem.removeOnTriggerEnter(this.thresholdTrigger)
      engine.removeEntity(this.thresholdTrigger)
      this.thresholdTrigger = undefined
    }

    if (this.portalLight) {
      engine.removeEntity(this.portalLight)
      this.portalLight = undefined
    }

    if (this.frameArrows) {
      engine.removeEntity(this.frameArrows)
      this.frameArrows = undefined
    }

    for (const layer of this.layers) {
      engine.removeEntity(layer)
    }
    this.layers = []

    if (this.audioAmb) {
      audioPlayer.stopSound(this.audioAmb)
      this.audioAmb = undefined
    }

    // infoCard children are parented to infoCard, so removeEntityWithChildren covers them all
    if (this.infoCard) {
      engine.removeEntityWithChildren(this.infoCard)
      this.infoCard = undefined
      this.infoBg = undefined
      this.thumbPlane = undefined
      this.infoLabel = undefined
      this.usersIcon = undefined
    }

    // Ensure doors are closed and counts are cleared
    const data = PortalData.getMutable(this.root)
    if (data.state !== PortalState.CLOSED) {
      this.fireDoorTween(0)
    }
    data.state = PortalState.CLOSED
    data.openCount = []
    data.ajarCount = []
    data.closeCount = []
  ...
  update(options: PortalOptions): void {
    ...
    if (this.enabled) {
      ...
      VisibilityComponent.createOrReplace(this.usersIcon!, { visible: hasUsers })

      if (this._thumbnail) {
        GltfContainer.createOrReplace(this.thumbPlane!, { src: 'assets/models/portals/event_card_thumbnail.glb' })
        GltfNodeModifiers.createOrReplace(this.thumbPlane!, {
          modifiers: [{
            ...
            material: {
              material: {
                $case: 'unlit', unlit: {
                  texture: Material.Texture.Common({ src: this._thumbnail }),
                  castShadows: false
                }
              }
            }
  ...
  private setPortalState(state: PortalState, angle: number, playOpen: boolean, playClose: boolean, playAmb: boolean): void {
    ...
    const pos = Transform.get(this.root).position
    if (playOpen) audioPlayer.playSingleSound({ audioClipUrl: 'assets/sounds/portals/doorOpen.mp3', volume: dBfsToLinear(-6), position: pos })
    if (playClose) audioPlayer.playSingleSound({ audioClipUrl: 'assets/sounds/portals/doorClose.mp3', volume: dBfsToLinear(-2), position: pos })
    if (playAmb) {
      if (!this.audioAmb) {
        this.audioAmb = audioPlayer.playSingleSound({ audioClipUrl: 'assets/sounds/portals/doorAmb.mp3', loop: true, volume: dBfsToLinear(-19), position: pos })
      }
    } else if (this.audioAmb) {
      audioPlayer.stopSound(this.audioAmb)
      this.audioAmb = undefined
    }
  ...
  private createTriggerZone(
    ...
    // sceneTriggerArea.setSphere(entity)
    // if (onEnter) {
    //   sceneTriggerArea.onTriggerEnter(entity, () => {
    //     const uid = this.getUserId(engine.PlayerEntity)
    //     if (!uid) return
    //     onEnter(`${engine.PlayerEntity}-${uid}`)
    //   })
    // }
    // if (onExit) {
    //   sceneTriggerArea.onTriggerExit(entity, () => {
    //     onExit(`${engine.PlayerEntity}`)
    //   })
    // }
  ...
  private createDoor(x: number, y: number, z: number, scaleX: number, scaleYZ: number, parent: Entity): Entity {
    ...
    GltfContainer.create(entity, { src: 'assets/models/portals/door.glb' })
    return entity
  }

  private addDisabledCollider(): void {
    if (this.disabledCollider) return
    this.disabledCollider = engine.addEntity()
    Transform.create(this.disabledCollider, {
      position: Vector3.create(FRAME_OFFSET.x, FRAME_OFFSET.y + 1.75, FRAME_OFFSET.z),
      rotation: Quaternion.Identity(),
      scale: Vector3.create(2.5, 3.5, 0.5),
      parent: this.portalBody,
    })
    MeshCollider.setBox(this.disabledCollider, ColliderLayer.CL_PHYSICS)
  }

  private removeDisabledCollider(): void {
    if (this.disabledCollider) {
      engine.removeEntity(this.disabledCollider)
      this.disabledCollider = undefined
    }
  }

  destroy(): void {
    if (this.enabled) {
      // sceneTriggerArea.remove(this.ajarTrigger!)
      // sceneTriggerArea.remove(this.openTrigger!)
      // sceneTriggerArea.remove(this.closeTrigger!)
